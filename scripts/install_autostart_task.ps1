<#!
.SYNOPSIS
Installs a Windows startup task for the GNEM web application.

.DESCRIPTION
Run once as a Windows administrator. Use a dedicated non-administrator Windows
service account, not an employee account. Windows asks for that account password;
...
$startScript = Join-Path $PSScriptRoot "start_factory_server.ps1"
...
Write-Host "Installed '$TaskName'. It will start GNEM after every Windows reboot."
