...
5. For the intended one-server factory deployment, open the [browser documentation index](documentation/index.html), especially the deployment, security, test-environment, and release-pipeline guides. Set `WEB_HOST=127.0.0.1` and `DB_HOST=127.0.0.1`; place an HTTPS reverse proxy in front of the app; do not expose Waitress/MySQL directly to employee devices or the internet.
